...
def CriterionReached(groupdataset):
	...
	for element in range(0, 89):
		temp = inputarray["Total"][(inputarray["Hour"] == indexer) & (inputarray["Total"] == 0)]
		if (len(temp) == 0):
			IndexMeans[element] = 0
		if (len(temp) > 0):
			IndexMeans[element] = (len(temp) / groupsize) * 100
		indexer = indexer + 1
		del temp
	IndexMeans = pd.DataFrame(IndexMeans)
	IndexErrors = pd.DataFrame(IndexErrors)
	Index = pd.concat([Hour, IndexMeans], axis = 1)
	Index.columns = ["Hour", "% of group"]
...
if (numberofgroups == 1):
	...
	plt.plot(Group1["Hour"], Group1["% of group"], Color=Color1, marker=",", linestyle="-")
	
	plt.title("Non Performers by Hour", fontsize=20)
	plt.xlabel("Hour", fontsize=16)
	plt.ylabel("% of group that made 0 total entries", fontsize=16)
